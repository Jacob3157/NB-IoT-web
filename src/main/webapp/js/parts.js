...
*/		
   		...
   		$.ajax({
			...
			success: function(data) {
				...
				len=data.total;
				code=data.rows;
				$str = '<tr><th>序号</th><th>配件编号</th><th>配件名称</th><th>适用机器</th><th>描述</th><th>库存</th><th>图片</th><th>操作</th></tr>';
			    for(var i = 0; i < code.length; i++){
			    	var num=i+begin;
			    	var imgurl="/Wechat/select/getPartsImg?imgurl="+code[i]['partsId'];
			    	$str += '<tr><td>'+num+'</td><td>'+code[i]['partsId']+'</td><td>'+code[i]['name']+'</td><td>'+code[i]['machineType']+'</td><td>'+code[i]['description']+'</td><td>'+code[i]['onStack']+'</td><td><img style="width:120px;height:60px"  src='+imgurl+'></td><td><button class="edit" id='+code[i]['partsId']+'>编辑</button></td></tr>';
			    }
				...
				display();
		   		...
		   		$(".edit").click(function(){
		   	         ...
		   	         post("/Wechat/select/singleParts",{partsId:partsId});
		   	    });
