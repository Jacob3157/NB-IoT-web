	...
	$(document).ready(function () {
		...
		function init(){
			$('#dg').datagrid({
                ...
                queryParams: {name:name,userProperty:usrProperty},
                columns: [[
                          ...
                          { field: 'property', title: "用户属性", width: '8%',
                          	editor:{type:'combobox',options:{valueField:'id',textField:'text',data:property}},
		...
		$(function(){
		   ...
		   if(level<=12){
               ...
               $("#dg").datagrid("hideColumn", "opt"); // 管理员属性小于13，不具有给予优惠的权利
		   }
		   ...
		   $("#searchName").click(function(){
				var queryParameter = $('#dg').datagrid("options").queryParams;  
	            queryParameter.name = $("#username").val();  
	            $('#dg').datagrid("options").url='/Wechat/select/userByName';
	            $("#dg").datagrid("load");  
			});
...
<body>
	...
	<div id="tb" style="padding:2px 5px;">
		...
		<a href="#" class="easyui-linkbutton" iconCls="icon-search" id="search" style="margin-right:50px">Search</a>
		用户名: <input class="easyui-textbox"   style="width:110px;margin-left:50px" id="username">
		<a href="#" class="easyui-linkbutton" iconCls="icon-search" id="searchName">search</a>
	</div>
